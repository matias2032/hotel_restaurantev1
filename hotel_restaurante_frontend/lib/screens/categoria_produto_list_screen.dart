...
class CategoriaProdutoListScreen extends StatefulWidget {
  const CategoriaProdutoListScreen({super.key});

  @override
  State<CategoriaProdutoListScreen> createState() =>
      _CategoriaProdutoListScreenState();
}

class _CategoriaProdutoListScreenState
    extends State<CategoriaProdutoListScreen> {
  _FiltroCategoriaProduto _filtro = _FiltroCategoriaProduto.todas;
  String _pesquisa = '';
  ...
  List<CategoriaProdutoModel> _filtrarCategorias(
    List<CategoriaProdutoModel> categorias,
    List<ProdutoModel> produtos,
  ) {
    return categorias.where((categoria) {
      final associados = _produtosAssociados(
        categoria,
        produtos,
      );

      final passaFiltro = switch (_filtro) {
        _FiltroCategoriaProduto.todas => true,
        _FiltroCategoriaProduto.activas => categoria.ativo,
        _FiltroCategoriaProduto.inactivas => !categoria.ativo,
        _FiltroCategoriaProduto.comProdutos => associados.isNotEmpty,
        _FiltroCategoriaProduto.semProdutos => associados.isEmpty,
      };
      ...
      final produtosAssociados = associados
          .map((produto) => produto.nome.toLowerCase())
          .join(' ');

      return nome.contains(termo) ||
          descricao.contains(termo) ||
          produtosAssociados.contains(termo);
    }).toList()
      ..sort((a, b) {
  ...
  List<ProdutoModel> _produtosAssociados(
    ...
    return produtos.where((produto) {
      return produto.categoriasProduto.any((categoriaResumo) {
        return categoriaResumo.idCategoriaProduto == idCategoria;
      });
    }).toList()
      ..sort(
  ...
  Future<void> _alterarEstado(
    ...
    final confirmado = await _confirmar(
      ...
      mensagem: novoEstado
          ...
          : 'A categoria será desactivada. Os produtos associados não serão eliminados.',
      ...
      builder: (context, provider, _) {
        ...
        return Scaffold(
          ...
          drawer: const AppSidebar(
            currentRoute: '/categorias-produto',
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Organize produtos por categorias e veja rapidamente os produtos associados.',
                  style: TextStyle(
                    color: _kMuted,
                    fontSize: 13,
                  ),
...
class _SearchAndFilters extends StatelessWidget {
  ...
  final _FiltroCategoriaProduto filtro;
  final ValueChanged<String> onPesquisaChanged;
  final ValueChanged<_FiltroCategoriaProduto> onFiltroChanged;
        ...
        children: [
          TextField(
            ...
            decoration: const InputDecoration(
              ...
              hintText: 'Pesquisar por nome, descrição ou produto associado',
            ),
          ),
          ...
          Align(
            ...
            child: Wrap(
              ...
              children: [
                _FilterChip(
                  label: 'Todas',
                  selected: filtro == _FiltroCategoriaProduto.todas,
                  onTap: () => onFiltroChanged(
                    _FiltroCategoriaProduto.todas,
                  ),
                ),
                _FilterChip(
                  label: 'Activas',
                  selected: filtro == _FiltroCategoriaProduto.activas,
                  onTap: () => onFiltroChanged(
                    _FiltroCategoriaProduto.activas,
                  ),
                ),
                _FilterChip(
                  label: 'Inactivas',
                  selected: filtro == _FiltroCategoriaProduto.inactivas,
                  onTap: () => onFiltroChanged(
                    _FiltroCategoriaProduto.inactivas,
                  ),
                ),
                _FilterChip(
                  label: 'Com produtos',
                  selected: filtro == _FiltroCategoriaProduto.comProdutos,
                  onTap: () => onFiltroChanged(
                    _FiltroCategoriaProduto.comProdutos,
                  ),
                ),
                _FilterChip(
                  label: 'Sem produtos',
                  selected: filtro == _FiltroCategoriaProduto.semProdutos,
                  onTap: () => onFiltroChanged(
                    _FiltroCategoriaProduto.semProdutos,
                  ),
                ),
              ],
            ),
...
class _CategoriaCard extends StatelessWidget {
  final CategoriaProdutoModel categoria;
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 14),
            ...
            children: [
              ...
              _SmallInfoChip(
                ...
                label:
                    '${associados.length} produto${associados.length == 1 ? '' : 's'}',
              ),
            ],
          ),
          const SizedBox(height: 12),
          ...
          if (associados.isEmpty)
            const Text(
              'Esta categoria ainda não está vinculada a nenhum produto.',
              style: TextStyle(
                color: _kMuted,
                fontSize: 13,
              ),
            ...
            Wrap(
              spacing: 8,
              runSpacing: 8,
              children: associados.map((produto) {
